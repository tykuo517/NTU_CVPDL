...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_info", type=str, default='text')
    parser.add_argument("--option", type=str, default='text')

    args = parser.parse_args()

    with open(f'../{args.image_info}') as json_file:
        datas = json.load(json_file)

    for data in datas:
        if args.option=='test-text':
            meta_list = [ 
        
                # - - - - - - - - GLIGEN on text grounding for generation - - - - - - - - # 
                dict(
                    ckpt = "../gligen_checkpoints/diffusion_pytorch_model_box_text.bin",  #"../gligen_checkpoints/checkpoint_generation_text.pth"
                    prompt = data['prompt_w_label'],
                    phrases = data['label'],
                    locations = data['bboxes'],
                    alpha_type = [0.3, 0.0, 0.7],
                    save_folder_name="generation_box_text_w_label"
                ), 
    
                dict(
                    ckpt = "../gligen_checkpoints/diffusion_pytorch_model_box_text.bin",  #"../gligen_checkpoints/checkpoint_generation_text.pth"
                    prompt = data['prompt_w_suffix'],
                    phrases = data['label'],
                    locations = data['bboxes'],
                    alpha_type = [0.3, 0.0, 0.7],
                    save_folder_name="generation_box_text_w_suffix"
                )
            ]
        elif args.option=='test-image':
            image_name=data['image']
            meta_list = [ 

                # - - - - - - - - GLIGEN on image grounding for generation - - - - - - - - # 
                dict(
                    ckpt = "../gligen_checkpoints/diffusion_pytorch_model_box_text_image.bin",
                    prompt = data['prompt_w_label'],
                    images = ['./inference_images/'+image_name],
                    phrases = data['label'],
                    locations = data['bboxes'],
            ...
            meta_list = [ 
                ...
                dict(
                    ...
                    locations = data['bboxes'],
                    alpha_type = [0.3, 0.0, 0.7],
                    save_folder_name="generation_box_text_w_label_data_gen"
                ), 
            ]
        elif args.option=='gen-image':
            image_name=data['image']
            meta_list = [ 
                ...
                dict(
                    ...
                    locations = data['bboxes'],
                    alpha_type = [1.0, 0.0, 0.0],
                    save_folder_name="generation_box_image_data_gen"
                ),
